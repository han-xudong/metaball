...
from metaball.models.torch.datamodule import MetaballDataModule
...
def main(
    ...
    if trainer.logger is not None and hasattr(trainer.logger, "version"):
        ...
        if logs:
            version_folder = os.path.join("./lightning_logs", sorted(logs, key=lambda x: int(x.split("_")[-1]))[-1])
        else:
            raise RuntimeError("No version folder found in ./lightning_logs.")

    if type == "conv":
        new_version_folder = (
            "./lightning_logs/ConvBallNet_" + time.strftime("%m%d-%H%M")
        )
    else:
        # Default to BallNet for other types
        new_version_folder = (
            "./lightning_logs/BallNet_" + time.strftime("%m%d-%H%M")
        )
    ...
    model_path = os.path.join(new_version_folder, "BallNet.pt")
